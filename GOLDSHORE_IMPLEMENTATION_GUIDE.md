...
- Start new Astro pages in `apps/site/src/pages`, modelling the hero/CTA structure after the current homepage stub for consistent typography, button styles, and CTA placement. 【F:apps/site/src/pages/index.astro†L1-L41】
...
- When authoring Astro components, match the grid utilities already used on the homepage (`grid grid-2`) until the shared theme is expanded; this keeps the responsive rules consistent across native Astro and static HTML surfaces. 【F:apps/site/src/pages/index.astro†L17-L37】
...
- Admin front-ends should rely on the Worker’s origin routing (see §3) to source environment-specific assets, ensuring staff always validate changes in preview/dev before touching production. 【F:apps/api-worker/src/index.ts†L69-L120】

## 3. Cloudflare Worker + GitHub Pages deployment
### 3.1 Environment separation & routing
- The Worker maps incoming hosts to the appropriate Pages origin (`production`, `preview`, `dev`) using environment variables that fall back to default URLs; keep this mapping up to date as domains change. 【F:apps/api-worker/src/index.ts†L10-L75】
- For bespoke staging domains, append them to the `PRODUCTION_ASSETS`, `PREVIEW_ASSETS`, or `DEV_ASSETS` variables—wildcards are ignored automatically, so list concrete URLs only. 【F:apps/api-worker/src/index.ts†L21-L63】
...
- Ensure `apps/api-worker/wrangler.toml` advertises the canonical Worker name (`api-goldshore`) and extend it with bindings (KV, D1, R2) as the stack grows. 【F:apps/api-worker/wrangler.toml†L1-L9】

### 3.3 Security, testing & observability
- CORS headers are currently mirrored from the request host; before shipping integrations, validate against each frontend origin and consider adding an allowlist lookup to `buildCorsHeaders` to prevent credential leaks. 【F:apps/api-worker/src/index.ts†L76-L116】
...
- Surface Worker provenance downstream by preserving the `x-served-by` header and augmenting logs/analytics collectors accordingly. 【F:apps/api-worker/src/index.ts†L112-L120】

### 3.4 Performance & caching strategy
- The Worker currently streams responses without modifying cache headers; layer in `Cache-Control`/`CF-Cache-Status` overrides at the Worker level once asset TTL requirements are defined, ensuring previews stay un-cached while production assets maximise edge storage. 【F:apps/api-worker/src/index.ts†L95-L120】
...
- Source images live in `apps/site/public/images/raw` and are transformed to AVIF/WEBP variants via the Sharp script before each deploy; fix the pipeline reuse bug by cloning the Sharp instance per format when you expand the script. 【F:packages/image-tools/process-images.mjs†L5-L29】【F:.github/workflows/deploy.yml†L36-L43】
...
- **goldshore.org (production)**
  - `/` – Astro homepage scaffold for hero + product navigation. 【F:apps/site/src/pages/index.astro†L1-L41】
...
- **preview.goldshore.org** – Mirrors production content for stakeholders; served by the Worker’s preview origin mapping. 【F:apps/api-worker/src/index.ts†L69-L75】
- **dev.goldshore.org** – Developer sandbox with experimental assets and feature flags; route via the Worker’s dev origin mapping. 【F:apps/api-worker/src/index.ts†L69-L75】
...
goldshore/
├─ apps/
│  ├─ api-worker/              # Cloudflare Worker router & GPT proxy
│  │  ├─ src/
│  │  └─ wrangler.toml         # Worker routes/bindings
│  └─ site/                    # Astro site (components, pages, styles, Pages functions)
│     ├─ public/
│     ├─ src/
...
- Global tokens (colours, spacing, motion) live in `apps/site/src/styles/theme.css`. Import the sheet in any layout to stay on
  brand. 【F:apps/site/src/styles/theme.css†L1-L117】
- The animated “glint” hero is encapsulated in `apps/site/src/components/Hero.astro`, complete with reduced-motion fallbacks. Use
  the `title`, `kicker`, and default slot to customise copy per page. 【F:apps/site/src/components/Hero.astro†L1-L68】
- `apps/site/src/components/Header.astro` provides a responsive header (desktop nav + mobile hamburger). Keep the structure to
  preserve accessibility semantics. 【F:apps/site/src/components/Header.astro†L1-L79】

## 2. Page architecture & SEO

- All pages share `apps/site/src/layouts/Base.astro`, which wires metadata, canonical URLs, OpenGraph/Twitter tags, skip links,
  and footer links. 【F:apps/site/src/layouts/Base.astro†L1-L46】
- Core routes ship with rich content out of the box:
  - Homepage hero + services overview. 【F:apps/site/src/pages/index.astro†L1-L63】
  - About, Team, Services (plus Bridgekeeper/Banproof detail pages) for storytelling depth. 【F:apps/site/src/pages/about.astro†L1-L54】【F:apps/site/src/pages/team.astro†L1-L48】【F:apps/site/src/pages/services/index.astro†L1-L45】
  - Blog index stub pointing to existing repo content. 【F:apps/site/src/pages/blog/index.astro†L1-L36】
  - Contact form scaffold with accessible controls. 【F:apps/site/src/pages/contact.astro†L1-L42】
  - Admin shell marked `noindex` for Cloudflare Access protection. 【F:apps/site/src/pages/admin/index.astro†L1-L39】
  - Privacy and Terms placeholders linked from the footer. 【F:apps/site/src/pages/privacy.astro†L1-L27】【F:apps/site/src/pages/terms.astro†L1-L26】
- Static SEO assets publish in `apps/site/public`, including `robots.txt`, `sitemap.xml`, and the logomark consumed by the header.
  【F:apps/site/public/robots.txt†L1-L3】【F:apps/site/public/sitemap.xml†L1-L12】【F:apps/site/public/logo.svg†L1-L11】

## 3. Image management

- Drop source artwork into `apps/site/public/images/raw`. Run `npm run process-images` (or `npm run build`) to emit optimised
...
- Base layout ships a skip link, consistent container spacing, and high-contrast palette baked into `theme.css`. 【F:apps/site/src/layouts/Base.astro†L24-L33】【F:apps/site/src/styles/theme.css†L19-L116】
- Components respect reduced-motion preferences and mobile breakpoints through scoped styles. 【F:apps/site/src/components/Hero.astro†L21-L34】【F:apps/site/src/components/Header.astro†L41-L63】
...
- `apps/api-worker/src/index.ts` forwards requests to the correct Pages project based on host prefix and stamps cache headers.
  Adjust the environment variables in `apps/api-worker/wrangler.toml` when preview origins change. 【F:apps/api-worker/src/index.ts†L1-L39】【F:apps/api-worker/wrangler.toml†L1-L9】
- Immutable caching is applied to hashed assets while HTML stays edge-cached for 10 minutes with stale-while-revalidate support.
  【F:apps/api-worker/src/index.ts†L22-L31】
...
- The admin shell is intentionally static—lock it behind Cloudflare Access with email allowlists or hardware keys. The Worker
  keeps the `x-served-by` header intact for provenance. 【F:apps/site/src/pages/admin/index.astro†L1-L39】【F:apps/api-worker/src/index.ts†L32-L36】
...
- Pages origins are declared per environment in `apps/site/wrangler.toml`; ensure the three Pages projects (`goldshore-org`, `-preview`,
  `-dev`) stay in sync with GitHub deployments. 【F:apps/site/wrangler.toml†L1-L23】

## 11. Developer ergonomics

- Quickstart: `npm install`, `npm run dev`, `npm run qa` before submitting PRs.
- Use the workspace deploy scripts (`deploy:pages`, `deploy:worker`) for manual pushes outside CI. 【F:package.json†L10-L17】

## 12. Launch checklist

1. Update or create content in `apps/site/src/pages`/`components` per sections 1–2.
